...
# Intentionally unquoted: when empty this expands to nothing rather than an
# empty argument; when set it's always the single word --delete. Using a
# plain string (not an array) here specifically so this works under old
# bash 3.2 (macOS's default /bin/bash), which errors on "${arr[@]}" for an
